...
class MockAPIClient: RequestExecutableProtocol, MockableResponse {
    ...
    func getRequest(
        endpoint: String,
        completionHandler: @escaping (Result<iOSCodingTask.WeatherDTO, iOSCodingTask.RequestError>) -> Void) {
            ...
            session.dataTask(with: request) { data, response, error in
                ...
                default:
                    ...
                    do {
                        let loadJson = try self.loadJSON(filename: fileName, modelType: WeatherDTO.self)
